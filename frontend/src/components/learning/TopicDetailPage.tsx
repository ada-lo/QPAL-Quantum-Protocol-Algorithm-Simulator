...
import { getTopicById, getTopicLabel, isProtocolTopic, type TopicCatalogEntry, type TopicInputConfig } from "@/lib/learning/topicCatalog"
import { buildInitialInputValues, buildPreviewCode, validateInput, type TopicInputValue } from "@/lib/learning/topicTemplateUtils"
...
export function TopicDetailPage({ mode }: TopicDetailPageProps) {
  ...
  function handleLoadCircuit() {
    ...
    navigate("/workspace", {
      state: {
        circuit: previewCode || catalogTemplate?.code || "",
        format: "custom",
        category: isProtocolTopic(topic.id) ? "protocol" : "algorithm",
        templateId: catalogTemplate?.id ?? topic.catalog_key,
      },
    })
  }
